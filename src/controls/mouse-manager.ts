...
type MouseMovedEvent = { x: number, y: number, dx: number, dy: number };
type MouseMovedCallback = (e: MouseMovedEvent) => any;

export class MouseManager {
   private _currentCoordinates: vec2 =  vec2.fromValues(0.0, 0.0);

   private _onMouseMovedCallbacks: Array<MouseMovedCallback> = new Array<MouseMovedCallback>();
   ...
   private updatePosition = (e: MouseEvent) => {
      ...
      this._onMouseMovedCallbacks.forEach(callback => callback({
