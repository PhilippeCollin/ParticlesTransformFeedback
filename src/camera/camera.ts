...
export class Camera {
  ...
  // Reusable vec3 for camera movement instructions
  private _moveVec: vec3 = vec3.create();
  ...
  moveInDirection(direction: Direction, speed: number) {
    console.log(direction);
    switch (direction) {
      case Direction.Forward:
        vec3.scale(this._moveVec, this._front, speed);
        break;
      case Direction.Back:
        vec3.scale(this._moveVec, this._front, -speed);
        break;
      case Direction.Left:
        vec3.scale(this._moveVec, this._right, -speed);
        break;
      case Direction.Right:
        vec3.scale(this._moveVec, this._right, speed);
        break;
      case Direction.Up:
        vec3.scale(this._moveVec, this._up, speed);
        break;
      case Direction.Down:
        vec3.scale(this._moveVec, this._up, -speed);
        break;
      default:
        vec3.set(this._moveVec, 0, 0, 0);
        break;
    }
    this.move(this._moveVec);
  }

  rotate({ pitch, yaw, roll }: Rotation) {
    if (pitch) {
  ...
  constructor(
    ...
    mat4.perspective(
      this._projection,
      (fovInDegrees / 360.0) * 2.0 * Math.PI,
      aspect,
      0.01,
      1000.0
    );
